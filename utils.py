...
def create_policies(args,
    ...
    policy_manager = PolicyManager() # policy dictionary, name to policy
    learner_class = None
    if training_config['algo'] == 'sac': # default to be True
    ...
    multiarm_motion_planner_learner = learner_class.options( # learner_class = SACLearner
...
def setup(args, config):
    if args.mode == 'behaviour-clone':
        return setup_behaviour_clone(
            args, config,
            obs_dim=get_observation_dimensions( # obs_dim of a single ur5
    ...
    if args.mode == 'enjoy':
        ...
        Env = BenchmarkEnv
    elif env_config['expert']['expert_on_fail']: # default to be true
        Env = RRTSupervisionEnv # used defaulted in training mode
        env_config['expert_root_dir'] = args.expert_waypoints
    else:
        Env = BaseEnv

    num_processes = config['training']['num_processes'] #16
    if num_processes < 0:
    ...
    # create targetLoader
    task_loader = None
    if args.tasks_path is not None: # load tasks
        print("[Setup] Creating task loader...")
        task_loader = ray.remote(TaskLoader).remote(
            root_dir=args.tasks_path, # load the tasks
            repeat=args.mode != 'benchmark',
            shuffle=True)
    else:
        print("[Setup] No tasks loaded. Generating tasks online.")
    config['training']['task_loader'] = task_loader # add the task loader into the config
    ...
    envs = [Env.remote(
            ...
            gui=args.gui,
            logger=logger)
            for _ in range(num_processes)] # 16
    ...
    obs_dim = get_observation_dimensions(training_config['observations']) # observation dimension of a single UR5
    action_dim = 6 # action dimension of a single ur5
    if training_config['centralized_policy']: # setting of control group
    ...
    policy_manager = create_policies(
        ...
        training_config=config['training'],
        action_dim=action_dim,
        actor_obs_dim=obs_dim, # dim of a single ur5
