...
class TaskManager:
    def __init__(self, config, colors, task_loader=None, training=True): # use task_loader as the argument
        self.current_task = None
        self.prioritized_task_loader = False
        self.use_task_loader = False
        if 'prioritized_task_loader' in config['environment']: # default to be false
            self.prioritized_task_loader = config[
                'environment']['prioritized_task_loader']
        if task_loader is not None: # default True
        ...
        if not self.static_tasks: # default False
            from .targetEnv import TargetEnv
            self.target_env = TargetEnv.remote(
                env_config=config['environment'],
                training_config=config['training'],
    ...
    def setup_next_task(self,
                        ...
                        min_task_ur5s_count = 1,
                        max_task_difficulty = 3,
                        min_task_difficulty = 1):
        if self.use_task_loader: # True
            while True:
            ...
            self.current_task = self.target_env.create_task.remote()
        self.set_timer(0.0)
...
class TaskLoader:
    def __init__(self, root_dir, shuffle=False, repeat=True):
        if exists(root_dir + 'all.txt'): # default to be False
            ...
            self.files = [abspath(file_name) # default to be True
